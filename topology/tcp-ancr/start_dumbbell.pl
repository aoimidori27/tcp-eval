...
# globals
my $leaf_kernel       = "/home/schulte/checkout/linux/linux-2.6.31.x-um-schulte/vmlinux-b3";
...
sub startup() {
	for (sort(keys(%nodes))) {
		system("sudo", "um_xm", "-k", $nodes{$_}, "-V", "hardy", "-r", "vmeshnode-hardy-initrd", "create", $ARGV[1] + $_);
	}
}
